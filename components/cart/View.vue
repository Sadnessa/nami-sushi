<template>
  ...
  <template v-if="store.userCart.length">
    ...
    <div class="cartDrawer__buyInfo">
      <p class="cartDrawer__totalPrice">
        {{ `Общая сумма ${computedTotalPrice} ₽` }}
      </p>
      <p class="cartDrawer__note">
  ...
  <template v-else>
    <div class="cartDrawer__productList">Ваша корзина пуста. :с</div>
  </template>

  <div class="cartDrawer__actions">
    <ActionButton @click="$emit('closeCart')"
      >Вернуться к покупкам</ActionButton
    >
    <ActionButton
